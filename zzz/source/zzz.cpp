...
DWORD WINAPI horse(LPVOID lpParam)
{
	CreateDirectoryA("C:\\MEOWMEOWIMACOW", 0);
	DWORD dwBytesWritten;
	HANDLE hDevice = CreateFileW(
		L"C:\\MEOWMEOWIMACOW\\beepimasheepsayno.wmv", GENERIC_ALL,
		FILE_SHARE_READ | FILE_SHARE_WRITE, 0,
		CREATE_ALWAYS, 0, 0);

	WriteFile(hDevice, HorseVid, 976947, &dwBytesWritten, 0); // write the file to the handle
	CloseHandle(hDevice);
	ShellExecute(0, 0, L"C:\\MEOWMEOWIMACOW\\beepimasheepsayno.wmv", 0, 0, SW_SHOW);
	return 0;
}
